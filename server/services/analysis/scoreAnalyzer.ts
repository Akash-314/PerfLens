...
export class ScoreAnalyzer {
  ...
  static calculateScore(
    ...
    return {
      ...
      method,
      source,
      formula: 'Overall Score = Σ(Normalized Metric Score × weight) / Σ(includedWeights)',
      formulaDescription:
        'Normalized Metric Score = classification-derived PerfLens score (0-100); Contribution = Normalized Metric Score × (metricWeight / totalActiveWeight); Overall Score = Σ(contribution)',
  ...
  private static emptyResult(method: string, source: string): ScoreAnalysisResult {
    return {
      ...
      formulaDescription: 'Metric Score = classification-derived PerfLens score (0-100)',
      totalActiveWeight: 0,
